...
async function 处理XHTTP请求(request, yourUUID) {
	...
	if (isSpeedTestSite(首包.hostname) && !启用SOCKS5反代) {
		try { reader.releaseLock() } catch (e) { }
					...
					while (pending.byteLength >= 5) {
						...
						if (remoteConnWrapper.socket || remoteConnWrapper.connectingPromise) {
							...
							if (判断是否是木马) {
								...
								if (isSpeedTestSite(hostname) && !启用SOCKS5反代) {
...
function 规范化Clash业务反代节点名(clash_yaml = '') {
	return String(clash_yaml)
		...
		.replace(/🇮🇪 DE → 🇮🇪 IE/g, '🇩🇪 DE → 🇮🇪 IE')
		.replace(/🇦🇹 DE → 🇦🇹 AT/g, '🇩🇪 DE → 🇦🇹 AT');
}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	config_JSON.订阅转换配置.SUBEMOJI = true;
	...
	if (!config_JSON.反代) config_JSON.反代 = 默认配置JSON.反代;
	if (config_JSON.反代.PROXYIP_SOURCE === undefined) config_JSON.反代.PROXYIP_SOURCE = "";
...
(() => {
	...
	function getSourceSelect() {
		return document.getElementById('edgetunnel-proxyip-source-select');
	}

	function getSelectedSource() {
		return getSourceSelect()?.value || '';
	}

	function setDisabledLook(element, disabled) {
		if (!element) return;
	...
	function syncProxyIPSourceMode() {
		const input = findProxyIPInput();
		const autoProxy = document.getElementById('autoProxy');
		const autoProxyLabel = document.querySelector('label[for="autoProxy"]');
		const select = getSourceSelect();
		if (!input || !select) return;
		const hasSource = !!select.value;
		if (hasSource) {
			const selectedLabel = select.options[select.selectedIndex]?.textContent || select.value;
			input.value = '业务池来源：' + selectedLabel;
			input.placeholder = '由业务池来源按国家自动匹配';
			setDisabledLook(input, true);
			if (autoProxy) {
				autoProxy.checked = false;
				setDisabledLook(autoProxy, true);
			}
			setDisabledLook(autoProxyLabel, true);
		} else {
			input.placeholder = 'proxyip.cmliussss.net';
			const autoChecked = !!autoProxy?.checked;
			setDisabledLook(input, autoChecked);
			setDisabledLook(autoProxy, false);
			setDisabledLook(autoProxyLabel, false);
		}
	}
	...
	function ensureSelector() {
		...
		select.id = 'edgetunnel-proxyip-source-select';
		select.title = '选择业务池 PROXYIP 来源';
		...
		status.id = 'edgetunnel-proxyip-source-status';
		status.style.fontSize = '12px';
		status.style.color = '#64748b';
		status.style.whiteSpace = 'nowrap';
		status.textContent = '业务池';

		select.addEventListener('change', () => {
			status.textContent = select.value ? '按业务池匹配' : '业务池';
			syncProxyIPSourceMode();
			if (typeof window.markModified === 'function') window.markModified('proxy');
		});
	...
	function patchAdminProxyFunctions() {
		if (window.__edgetunnelProxyipSourcePatched) return;
		window.__edgetunnelProxyipSourcePatched = true;
		let savedSourceLoaded = false;
		async function loadSavedSource() {
			if (savedSourceLoaded) return;
			savedSourceLoaded = true;
			try {
				const response = await window.fetch('/admin/config.json?_proxyip_source=' + Date.now(), {
					headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate' },
				});
				if (!response.ok) return;
				const config = await response.json();
				const source = config?.反代?.PROXYIP_SOURCE || '';
				const select = getSourceSelect();
				if (select) {
					select.value = source;
					const status = document.getElementById('edgetunnel-proxyip-source-status');
					if (status) status.textContent = source ? '按业务池匹配' : '业务池';
					syncProxyIPSourceMode();
				}
			} catch (error) {
				console.warn('[edgetunnel] PROXYIP source config load failed:', error);
			}
		}
		...
		window.fetch = async function(input, init) {
			...
			if (method === 'POST' && /\\/admin\\/config\\.json(?:\\?|$)/.test(requestUrl) && getSourceSelect()) {
				try {
					const nextInit = { ...(init || {}) };
					const rawBody = nextInit.body !== undefined ? nextInit.body : (input && input.body);
					if (typeof rawBody === 'string') {
						const config = JSON.parse(rawBody);
						if (!config.反代) config.反代 = {};
						if (!config.反代.SOCKS5) config.反代.SOCKS5 = {};
						config.反代.PROXYIP_SOURCE = source;
						if (source) {
							config.反代.PROXYIP = 'auto';
							config.反代.SOCKS5.启用 = null;
							config.反代.SOCKS5.全局 = false;
							config.反代.SOCKS5.账号 = '';
						}
						nextInit.body = JSON.stringify(config);
						return originalFetch(input, nextInit);
					}
				} catch (error) {
					console.warn('[edgetunnel] PROXYIP source save patch failed:', error);
				}
			}
			return originalFetch(input, init);
		};
		const originalApply = window.applyProxyConfigToForm;
		if (typeof originalApply === 'function') {
			window.applyProxyConfigToForm = function(...args) {
		...
		const originalUpdateProxyMode = window.updateProxyMode;
		if (typeof originalUpdateProxyMode === 'function') {
			window.updateProxyMode = function(...args) {
				const result = originalUpdateProxyMode.apply(this, args);
				ensureSelector();
				syncProxyIPSourceMode();
				return result;
			};
		}
		const originalToggleAutoProxy = window.toggleAutoProxy;
		if (typeof originalToggleAutoProxy === 'function') {
			window.toggleAutoProxy = function(...args) {
				if (getSelectedSource()) {
					const autoProxy = document.getElementById('autoProxy');
					if (autoProxy) autoProxy.checked = false;
					syncProxyIPSourceMode();
					if (typeof window.markModified === 'function') window.markModified('proxy');
					return;
				}
				const result = originalToggleAutoProxy.apply(this, args);
				syncProxyIPSourceMode();
				return result;
			};
		}
...
async function 获取AdminProxyIP来源(来源) {
	const 来源URL = 获取反代IP池来源URL(来源) || 获取反代IP池来源URL('github');
	try {
...
function 获取反代IP池来源URL(来源) {
	if (来源 === 'github') return 'https://raw.githubusercontent.com/GuardSkill/CFOpt/refs/heads/main/proxyip-best.txt';
	if (来源 === 'zip') return 'https://zip.cm.edu.kg/all.txt';
	if (typeof 来源 === 'string' && /^https?:\/\//i.test(来源)) return 来源;
	return '';
}

function 获取业务反代IP来源(config = {}) {
	const 来源 = config?.反代?.PROXYIP_SOURCE || config?.反代?.ProxyIPSource || '';
...
async function 获取国家反代IP池(源 = 'https://raw.githubusercontent.com/GuardSkill/CFOpt/refs/heads/main/proxyip-best.txt', 每国数量 = 10, 缓存毫秒 = 6 * 60 * 60 * 1000) {
	...
	const 允许国家 = new Set(['AT', 'AU', 'DE', 'GB', 'HK', 'IE', 'JP', 'KR', 'SG']);
	const 国家反代IP池 = {};
	try {
...
function 提取优选IP入口国家(原始地址) {
	...
	const 匹配 = 备注.match(/\b([A-Za-z]{2})\b/);
	return 匹配 ? 匹配[1].toUpperCase() : 'SG';
}

function 提取优选IP测速标签(备注) {
	const 匹配 = 备注.match(/\[[^\]]+\]/);
	return 匹配 ? 匹配[0] : 备注;
}
...
function 生成业务反代优选IP(完整优选IP, 国家反代IP池) {
	const 反代链路 = [
		{ 入口国家: 'DE', 反代国家: 'IE', 入口旗帜: '🇩🇪', 反代旗帜: '🇮🇪' },
		{ 入口国家: 'DE', 反代国家: 'AT', 入口旗帜: '🇩🇪', 反代旗帜: '🇦🇹' },
		{ 入口国家: 'GB', 反代国家: 'IE', 入口旗帜: '🇬🇧', 反代旗帜: '🇮🇪' },
		{ 入口国家: 'HK', 反代国家: 'HK', 入口旗帜: '🇭🇰', 反代旗帜: '🇭🇰' },
		{ 入口国家: 'JP', 反代国家: 'JP', 入口旗帜: '🇯🇵', 反代旗帜: '🇯🇵' },
		{ 入口国家: 'KR', 反代国家: 'KR', 入口旗帜: '🇰🇷', 反代旗帜: '🇰🇷' },
		{ 入口国家: 'SG', 反代国家: 'SG', 入口旗帜: '🇸🇬', 反代旗帜: '🇸🇬' },
	];
	const 结果 = [];
	for (const 原始地址 of 完整优选IP) {
		const 入口国家 = 提取优选IP入口国家(原始地址);
		const 原备注 = 提取优选IP备注(原始地址) || 原始地址.split('#')[0];
		const 匹配链路 = 反代链路.filter(item => item.入口国家 === 入口国家);
		if (匹配链路.length === 0) continue;
		for (const 链路 of 匹配链路) {
			const 反代池 = 国家反代IP池[链路.反代国家] || [];
			if (反代池.length === 0) continue;
			const 反代参数 = 反代池.join(',');
			const 测速标签 = 提取优选IP测速标签(原备注);
			const 新备注 = 链路.入口国家 === 链路.反代国家
				? `${链路.入口旗帜} ${链路.入口国家} ↪ ${测速标签} $proxyip=${反代参数}`
				: `${链路.入口旗帜} ${链路.入口国家} → ${链路.反代旗帜} ${链路.反代国家} ${测速标签} $proxyip=${反代参数}`;
