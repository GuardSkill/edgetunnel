...
proxies:
  ...
  - {name: "🇱🇺 DE → 🇱🇺 LU [北京测速#02 ip.zip]", server: 203.0.113.2}
proxy-groups:
  - name: 🇩🇪 Germany Entry + 🇱🇺 LU Proxy
    proxies:
      - 🇦🇺 DE → 🇦🇺 AU [北京测速#01 ip.zip]
      - 🇱🇺 DE → 🇱🇺 LU [北京测速#02 ip.zip]
`;
...
assert.equal(result.includes('🇱🇺 DE → 🇱🇺 LU'), false);
assert.equal((result.match(/🇩🇪 DE → 🇱🇺 LU/g) || []).length, 2);
